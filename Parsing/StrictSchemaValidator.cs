...
namespace Converter.Parsing
{
    ...
    public static class StrictSchemaValidator
    {
        ...
        {
            // ВПК.xlsx: Фото | Местонахождение | (пустая) | ФИО | Должность | E-mail: | Контактный телефон | Внутр. номер телефона
            ["ВПК"] = new[]
            {
                Canon("Фото"),
                Canon("Местонахождение"),
                Canon(""), // пустая колонка
                Canon("ФИО"),
                Canon("Должность"),
                Canon("E-mail:"),
            ...
            // ВЗК.xlsx: № | Организация | Структурное подразделение/ департамент | ФИО | Должность | Электронный адрес | Код города | Городской номер | Мобильный номер | Внутренний телефон
            ["ВЗК"] = new[]
            {
                Canon("№"),
                Canon("Организация"),
            ...
            // ВИЦ.xlsx: № | Организация | Структурное подразделение/ департамент | ФИО | Должность | Электронный адрес | Код города | Городской номер | Мобильный номер | Дополнительный номер/ e-mail | Внутренний телефон
            ["ВИЦ"] = new[]
            {
                Canon("№"),
                Canon("Организация"),
            ...
            // ЗЗГТ.xlsx: № | Организация | Структурное подразделение/ департамент | ФИО | Должность | Электронный адрес | Код города | Городской номер | Мобильный номер | Внутренний телефон
            ["ЗЗГТ"] = new[]
            {
                Canon("№"),
                Canon("Организация"),
        ...
        public static ValidationResult ValidateStrict(string button, ISheet sheet)
        {
            ...
            // Дополнительная проверка по названию листа для различения ВЗК и ЗЗГТ
            if (button == "ВЗК" && !sheet.SheetName.ToLowerInvariant().Contains("завод"))
                return new(false, $"Лист называется '{sheet.SheetName}', но ожидается название с 'завод' для ВЗК.", -1, -1, 0);
                
            if (button == "ЗЗГТ" && !sheet.SheetName.ToLowerInvariant().Contains("ззгт"))
                return new(false, $"Лист называется '{sheet.SheetName}', но ожидается название с 'ззгт' для ЗЗГТ.", -1, -1, 0);
